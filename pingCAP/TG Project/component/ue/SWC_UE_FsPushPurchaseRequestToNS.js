...
define(["N/runtime", "N/search", "../../common/SWC_OMS_Utils", "N/file", 'N/record', 'N/currency',
    ...
    function (runtime, search, SWCommons, file, record, currency, SWC_FsPushApprovalCmn,
        ...
        function syncFsPushPurchApply(options) {
            ...
            log.audit("submitFlag", submitFlag)
            ...
            var custrecord_pr_extramemo = newRec.getValue({fieldId: "custrecord_pr_extramemo"});//申请理由
            var custrecord_pr_ven_single_rationale = newRec.getValue({fieldId: "custrecord_pr_ven_single_rationale"});//单一供应商理由
            var custrecord_pr_vendor_first = newRec.getValue({fieldId: "custrecord_pr_vendor_first"});//首选供应商
            var custrecord_pr_vendor_sec = newRec.getValue({fieldId: "custrecord_pr_vendor_sec"});//次选供应商
            var custrecord_pr_vendor_third = newRec.getValue({fieldId: "custrecord_pr_vendor_third"});//末选供应商
            var custrecord_pr_vendor_memo = newRec.getValue({fieldId: "custrecord_pr_vendor_memo"});//首选供应商入选理由，供应商报价
            var custrecord_pr_vendor_memo2 = newRec.getValue({fieldId: "custrecord_pr_vendor_memo2"});//次选供应商入选理由，供应商报价
            var custrecord_pr_vendor_memo3 = newRec.getValue({fieldId: "custrecord_pr_vendor_memo3"});//末选供应商入选理由，供应商报价
            var custrecord_prwf_onlyflag = false;//是否单一供应商
            if(custrecord_pr_ven_single_rationale)custrecord_prwf_onlyflag = true;
            var curCurrency = newRec.getText({fieldId:"custrecord_pr_currency"});// 币种
            // 将币种金额转换成美元
            var rate = currency.exchangeRate({source: curCurrency, target: 'USD'});

            var bodyPushFormObj = {};//需要生成单据的数据
            for (var i = 0; i < prsLineCount; i++) {
                // 已经推送成功的货品，跳过处理
                var approvalStatus = newRec.getSublistValue({
                    sublistId: "recmachcustrecord_prs_field",
                    fieldId: "custrecord_prs_line_status",
                    line: i
                });
                ...
                var budgetDepartment = newRec.getSublistValue({sublistId: "recmachcustrecord_prs_field", fieldId: "custrecord_prs_budget_department", line: i});// 预算归属部门
                var itemValue = newRec.getSublistValue({sublistId: "recmachcustrecord_prs_field", fieldId: "custrecord_prs_item", line: i});// 费用类型
                var typeValue = newRec.getSublistValue({sublistId: "recmachcustrecord_prs_field", fieldId: "custrecord_prs_expense_type", line: i});// 费用类别
                var totalamountValue = newRec.getSublistValue({sublistId: "recmachcustrecord_prs_field", fieldId: "custrecord_prs_totalamount", line: i});// 金额
                var assetitValue = newRec.getSublistValue({sublistId: "recmachcustrecord_prs_field", fieldId: "custrecord_prs_feishu_assetit", line: i});// 资产分类
                var prspro = newRec.getSublistValue({sublistId: "recmachcustrecord_prs_field", fieldId: "custrecord_prs_pro", line: i});// 项目

                var usdAmount = Number(decimal.mulN(totalamountValue, rate).toFixed(8));//金额(USD)
                ...
                if(bodyPushFormObj.hasOwnProperty(itemLineKey)){
                    ...
                    bodyPushFormObj[itemLineKey]["custrecord_prwf_amount_usd"] = decimal.addN(Number(bodyPushFormObj[itemLineKey]["custrecord_prwf_amount_usd"]), Number(usdAmount));

                }else {
                    //将采购申请数据赋值到JSON中（后期将JSON数据赋值到采购申请审批单据中）
                    var itemJson = {};
                    itemJson["department"] = budgetDepartment;
                    itemJson["item"] = itemValue;
                    itemJson["expensetype"] = typeValue;
                    itemJson["totalamount"] = totalamountValue;
                    itemJson["assetit"] = assetitValue;
                    itemJson["prspro"] = prspro;
                    itemJson["empId"] = empId;

                    itemJson["custrecord_pr_extramemo"] = custrecord_pr_extramemo;
                    itemJson["custrecord_pr_ven_single_rationale"] = custrecord_pr_ven_single_rationale;
                    itemJson["custrecord_pr_vendor_first"] = custrecord_pr_vendor_first;
                    itemJson["custrecord_pr_vendor_sec"] = custrecord_pr_vendor_sec;
                    itemJson["custrecord_pr_vendor_third"] = custrecord_pr_vendor_third;
                    itemJson["custrecord_pr_vendor_memo"] = custrecord_pr_vendor_memo;
                    itemJson["custrecord_pr_vendor_memo2"] = custrecord_pr_vendor_memo2;
                    itemJson["custrecord_pr_vendor_memo3"] = custrecord_pr_vendor_memo3;
                    itemJson["custrecord_prwf_onlyflag"] = custrecord_prwf_onlyflag;
                    itemJson["custrecord_prwf_amount_usd"] = usdAmount;
            ...
            log.audit("bodyPushFormObj",bodyPushFormObj);
            for (var key in bodyPushFormObj) {

                // {"msg": "", "instanceCode": ""} 创建采购申请审批单据
            ...
            log.audit("pushFsApprovalResultObj", pushFsApprovalResultObj)
            ...
            for (var i = 0; i < prsLineCount; i++) {
                ...
                if (approvalStatus == SWC_CONFIG_DATA.configData().WORKFLOW_STATUS_TO_BE_SUBMITTED
                    ...
                    if (pushFsApprovalResultObj.hasOwnProperty(lineKey)) {
                        ...
                        // 采购审批中间表
                        purchReqRec.setSublistValue({sublistId: "recmachcustrecord_prs_field", fieldId: "custrecord_prs_ns_approval", value: instanceCode, line: i});
                    }
                }
            }
        ...
        function getBodyPushFormAry(options) {
            ...
            for (var i = 0; i < prsLineCount; i++) {
                ...
                log.audit("!fsOpenId || errFlag 跳过处理", {"fsOpenId": fsOpenId, errFlag: errFlag});
                if (!fsOpenId || errFlag) {
                    exceptionFlag = true;
                    // 记录错误信息
