...
define(["N/runtime", "N/search", "../../common/SWC_OMS_Utils", "N/file", 'N/record', 'N/currency',
        "../../common/SWC_FsPushApprovalCmnToNS", "../../common/SWC_CONFIG_DATA", "../../lib/decimal", "N/format", "N/url"],
    ...
    function (runtime, search, SWCommons, file, record, currency, SWC_FsPushApprovalCmn,
        ...
        function syncFsPushVendorApply(options) {
            ...
            log.audit("apsLineCount", apsLineCount)
            ...
            var employee = newRec.getValue({fieldId: "custrecord_ap_employee"});//提交人
            var custrecord_ap_reason_description = newRec.getValue({fieldId: "custrecord_ap_reason_description"});//事由描述
            var custrecord_ap_payment_method = newRec.getValue({fieldId: "custrecord_ap_payment_method"});//付款方式
            var custrecord_ap_payment_period = newRec.getValue({fieldId: "custrecord_ap_payment_period"});//付款周期
            var custrecord_ap_vendorname = newRec.getValue({fieldId: "custrecord_ap_vendorname"});//供应商名称
            var custrecord_ap_currency = newRec.getValue({fieldId: "custrecord_ap_currency"});//币种
            var custrecord_ap_taxcode = newRec.getValue({fieldId: "custrecord_ap_taxcode"});//税码
            var custrecord_ap_vendor_bankname = newRec.getValue({fieldId: "custrecord_ap_vendor_bankname"});//VENDOR BANK NAME
            var custrecord_ap_vendor_bank_accountno = newRec.getValue({fieldId: "custrecord_ap_vendor_bank_accountno"});//VENDOR BANK ACCOUNT NO.
            var custrecord_ap_vendorbank_citystate = newRec.getValue({fieldId: "custrecord_ap_vendorbank_citystate"});//VENDOR BANK CITY OR STATE
            var custrecord_ap_swiftcode = newRec.getValue({fieldId: "custrecord_ap_swiftcode"});//SWIFT CODE
            var custrecord_ap_routing_transitno = newRec.getValue({fieldId: "custrecord_ap_routing_transitno"});//ROUTING & TRANSIT NO.
            var custrecord_ap_invoiceno = newRec.getValue({fieldId: "custrecord_ap_invoiceno"});//INVOICE编号
            var custrecord_ap_fullname = newRec.getValue({fieldId: "custrecord_ap_fullname"});//收款人全名
            var custrecord_ap_address = newRec.getValue({fieldId: "custrecord_ap_address"});//收款人收件地址
            var custrecord_ap_phone = newRec.getValue({fieldId: "custrecord_ap_phone"});//收款人联系电话
            var custrecord_ap_address_payee = newRec.getValue({fieldId: "custrecord_ap_address_payee"});//收款人邮箱
            var custrecord_ap_invoice_attachment = newRec.getValue({fieldId: "custrecord_ap_invoice_attachment"});//发票附件1
            var custrecord_ap_invoice_attachment2 = newRec.getValue({fieldId: "custrecord_ap_invoice_attachment2"});//发票附件2
            var custrecord_ap_invoice_attachment3 = newRec.getValue({fieldId: "custrecord_ap_invoice_attachment3"});//发票附件3
            var custrecord_ap_over_reason = newRec.getValue({fieldId: "custrecord_ap_over_reason"});//超申请理由
            var custrecord_ap_actul_paytime = newRec.getValue({fieldId: "custrecord_ap_actul_paytime"});//付款日期
            var custrecord_ap_expected_paytime = newRec.getValue({fieldId: "custrecord_ap_expected_paytime"});//期望付款时间
            var custrecord_ap_subsidary = newRec.getValue({fieldId: "custrecord_ap_subsidary"});//付款主体


            // 合同或报价单附件（【采购订单】.采购合同文件）
            // 取得采购订单单号
            var purchOrdIntlId = newRec.getValue({fieldId: "custrecord_ap_number"});
            var fileIdAryFirst = "";//合同或报价单附件
            var poAmount = 0;//采购订单金额
            var poPaidAmount = 0;//已付款金额
            var poUnpayAmount = 0;//采购订单未付金额
            if (purchOrdIntlId) {
                // 采购订单存在的场合，取得采购合同文件
                // var fileIdAry = SWC_FsPushApprovalCmn.schPurchRelatedFile(purchOrdIntlId);// 附件ID们
                // if(fileIdAry.length>0)fileIdAryFirst = fileIdAry[0];//取第一条
                // 采购订单金额：采购订单号为空的场合，默认值0；以外的场合，根据采购订单号检索采购订单，计算采购订单总金额
                poAmount = SWC_FsPushApprovalCmn.getPOAmtForCurr(purchOrdIntlId);
                // 已付款金额：采购订单号为空的场合，默认值0；以外的场合，根据采购订单号检索账单，计算账单明细行金额（外币）
                poPaidAmount = SWC_FsPushApprovalCmn.getPoBillAmtForCurr(purchOrdIntlId);
                // 采购订单未付金额：采购订单金额 - 采购订单已付款金额
                poUnpayAmount = decimal.subN(poAmount, poPaidAmount);
            }
            var supportdoc = newRec.getValue({fieldId: "custrecord_ap_other_supportdoc"});//结算单等其他支持性文件
            log.audit("结算单等其他支持性文件",supportdoc);
            if(supportdoc)fileIdAryFirst = supportdoc;

            var bodyPushFormObj = {};//需要生成单据的数据
            for (var i = 0; i < apsLineCount; i++) {
                ...
                var budgetDepartment = newRec.getSublistValue({sublistId: "recmachcustrecord_aps_field", fieldId: "custrecord_aps_department", line: i});//预算归属部门
                var itemValue = newRec.getSublistValue({sublistId: "recmachcustrecord_aps_field", fieldId: "custrecord_aps_item", line: i});// 费用类型(货品)
                var expensetype = newRec.getSublistValue({sublistId: "recmachcustrecord_aps_field", fieldId: "custrecord_aps_catagorytype", line: i});//费用类别
                var totalamountValue = newRec.getSublistValue({sublistId: "recmachcustrecord_aps_field", fieldId: "custrecord_aps_totalamount", line: i});//总金额
                var apspro = newRec.getSublistValue({sublistId: "recmachcustrecord_aps_field", fieldId: "custrecord_aps_pro", line: i});//项目

                var itemLineKey = budgetDepartment + "_" + itemValue;
                log.audit("itemLineKey", itemLineKey);
                if(bodyPushFormObj.hasOwnProperty(itemLineKey)){
                    bodyPushFormObj[itemLineKey]["totalamount"] = Number(bodyPushFormObj[itemLineKey]["totalamount"])+Number(totalamountValue);

                }else {
                    //将供应商账单申请数据赋值到JSON中（后期将JSON数据赋值到采购付款申请审批单据中）
                    var itemJson = {};
                    itemJson["department"] = budgetDepartment;
                    itemJson["item"] = itemValue;
                    itemJson["expensetype"] = expensetype;
                    itemJson["totalamount"] = totalamountValue;
                    itemJson["apspro"] = apspro;
                    itemJson["empId"] = employee;

                    itemJson["custrecord_ap_reason_description"] = custrecord_ap_reason_description;
                    itemJson["custrecord_ap_payment_method"] = custrecord_ap_payment_method;
                    itemJson["custrecord_ap_payment_period"] = custrecord_ap_payment_period;
                    itemJson["custrecord_ap_vendorname"] = custrecord_ap_vendorname;
                    itemJson["custrecord_ap_currency"] = custrecord_ap_currency;
                    itemJson["custrecord_ap_taxcode"] = custrecord_ap_taxcode;
                    itemJson["custrecord_ap_vendor_bankname"] = custrecord_ap_vendor_bankname;
                    itemJson["custrecord_ap_vendor_bank_accountno"] = custrecord_ap_vendor_bank_accountno;
                    itemJson["custrecord_ap_vendorbank_citystate"] = custrecord_ap_vendorbank_citystate;
                    itemJson["custrecord_ap_swiftcode"] = custrecord_ap_swiftcode;
                    itemJson["custrecord_ap_routing_transitno"] = custrecord_ap_routing_transitno;
                    itemJson["custrecord_ap_invoiceno"] = custrecord_ap_invoiceno;
                    itemJson["custrecord_ap_fullname"] = custrecord_ap_fullname;
                    itemJson["custrecord_ap_address"] = custrecord_ap_address;
                    itemJson["custrecord_ap_phone"] = custrecord_ap_phone;
                    itemJson["custrecord_ap_address_payee"] = custrecord_ap_address_payee;
                    itemJson["custrecord_ap_address_payee"] = custrecord_ap_address_payee;
                    itemJson["custrecord_ap_invoice_attachment"] = custrecord_ap_invoice_attachment;
                    itemJson["custrecord_ap_invoice_attachment2"] = custrecord_ap_invoice_attachment2;
                    itemJson["custrecord_ap_invoice_attachment3"] = custrecord_ap_invoice_attachment3;
                    itemJson["custrecord_ap_over_reason"] = custrecord_ap_over_reason;
                    itemJson["custrecord_ap_actul_paytime"] = custrecord_ap_actul_paytime;
                    itemJson["fileIdAryFirst"] = fileIdAryFirst;
                    itemJson["poAmount"] = poAmount;
                    itemJson["poPaidAmount"] = poPaidAmount;
                    itemJson["poUnpayAmount"] = poUnpayAmount;
                    itemJson["purchOrdIntlId"] = purchOrdIntlId;
                    itemJson["custrecord_ap_expected_paytime"] = custrecord_ap_expected_paytime;
                    itemJson["custrecord_ap_subsidary"] = custrecord_ap_subsidary;

                    bodyPushFormObj[itemLineKey] = itemJson;
                }
            }
            ...
            for (var key in bodyPushFormObj) {
                ...
                var pushApprovalResult = SWC_FsPushApprovalCmn.pushFsApproval({
                    ...
                    info:bodyPushFormObj[key]
                });

                pushFsApprovalResultObj[key] = pushApprovalResult;
            }
            log.audit("推送处理结果", pushFsApprovalResultObj)
            ...
            for (var i = 0; i < apsLineCount; i++) {
                ...
                if (pushFsApprovalResultObj.hasOwnProperty(lineKey)) {
                    ...
                    log.audit("instanceCode", instanceCode)
                    // 飞书审批推送结果
                    vendorBillApplyRec.setSublistValue({sublistId: "recmachcustrecord_aps_field", fieldId: "custrecord_aps_code", value: msg, line: i});
                    // 采购审批中间表
                    vendorBillApplyRec.setSublistValue({sublistId: "recmachcustrecord_aps_field", fieldId: "custrecord_aps_ns_approval", value: instanceCode, line: i});
                }
            }
